        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {buildItems.map((item) => (
            <button
              ...
              className="group relative text-left overflow-hidden border border-white/5 border-b-2 border-b-transparent hover:border-brand-orange transition-all duration-300"
            >
              {/* Thumbnail */}
              <div className="relative aspect-[4/5] overflow-hidden bg-charcoal-850">
                ...
                {/* Hover arrow */}
                <div className="absolute top-4 right-4 bg-white/0 group-hover:bg-brand-orange text-white p-2 rounded-none transition-all duration-300">
                  <ArrowUpRight className="h-4 w-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                </div>

                {/* Caption */}
                <div className="absolute bottom-0 left-0 right-0 p-5">
                  <h3 className="text-base font-extrabold uppercase tracking-wider text-white mb-1">
